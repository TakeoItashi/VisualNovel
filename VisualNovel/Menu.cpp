						...
						if (m_keywords[i] == "Button" && m_keywords[i + 1] == "{") {
							...
							for (i; i < m_keywords.size() - 1;) {
								...
								if (m_keywords[i] == "Type:") {
									...
									AddFunctions(menuItem.Button, atoi(m_keywords[i + 1].c_str()), new std::string[1] {"Test"});
									i += 2;
									continue;
								}
...
void Menu::AddFunctions(Button* _button, int _type, std::string _args[] = nullptr) {
	switch (_type) {
	...
	case MenuItemType::Custom:
		...
		break;
	case MenuItemType::SaveSlot:
		_button->m_delegateFunction = std::bind(Game::GetInstance()->SaveFunction, _button, m_saveSlotIndex);
		++m_saveSlotIndex;
		break;
	case MenuItemType::BackToGame:
		_button->m_delegateFunction = std::bind(&Game::GetInstance()->BackToGame, _button);
		break;
	case MenuItemType::BackToPauseMenu:
		_button->m_delegateFunction = std::bind(&Game::GetInstance()->ShowPauseMenu);
		break;
	case MenuItemType::LoadSlot:
		_button->m_delegateFunction = std::bind(Game::GetInstance()->LoadFunction, _button, m_saveSlotIndex);
		++m_saveSlotIndex;
		break;
	case MenuItemType::BackToMainMenu:
		_button->m_delegateFunction = std::bind(&Game::GetInstance()->ResetGameToMainMenu);
		break;
	case MenuItemType::ShowSaveMenu:
		_button->m_delegateFunction = std::bind(&Game::GetInstance()->ShowSaveMenu, _button);
		break;
	case MenuItemType::ShowLoadMenu:
		_button->m_delegateFunction = std::bind(&Game::GetInstance()->ShowLoadMenu, _button);
		break;
	}
}